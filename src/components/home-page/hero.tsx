      ...
      <div className="bg-[#211C24] text-white">
        <div className="max-w-screen-xl mx-auto px-6 sm:px-8 md:px-10 lg:px-12">
          ...
          <div className="block md:hidden text-center animate-fadeIn">
            <p className="text-sm text-gray-400 pt-28">{t('hero.tagline')}</p>
            <h2 className="text-6xl sm:text-7xl animate-slideUp">
              {t('hero.heading')} <span className="font-bold">{t('hero.subHeading')}</span>
            </h2>
            <p className="text-sm text-gray-300 mb-10 animate-slideUp">{t('hero.description')}</p>
            <Button text={t('hero.buttonText')} to="/product" className="animate-slideUp" />
            <div className="mt-10">
              <Image
          ...
          <div className="hidden md:flex md:flex-row items-center animate-fadeIn">
            <div className="w-full md:w-3/5 lg:w-4/5 px-4 md:px-8 lg:px-14 animate-slideUp">
              <p className="text-base text-gray-400">{t('hero.tagline')}</p>
              <h2 className="text-6xl sm:text-7xl md:text-8xl lg:text-[90px] leading-tight">
                {t('hero.heading')} <span className="font-bold">{t('hero.subHeading')}</span>
              </h2>
              <p className="text-base text-gray-300 max-w-md mb-10">{t('hero.description')}</p>
              <Button text={t('hero.buttonText')} to="/product" />
            </div>
